...
def create_graph_with_weight(points, normCount):
    ...
    calculate length of edge, remove edges > bottom 50%
    
    use triangle remove longest edge! 
    '''
    edges = {}   
    var = normCount.var()
...
def read_spatial_expression(file):
    ...
    :rtype: spatial coordinates (n, 2); data: shape (n, m); 
    '''
...
def plot_voronoi(geneID, coord, count, 
    ...
    :param file: spatial coordinates (n, 2); normalized count: shape (n); 
    '''
    points = coord
    tempPoints = points
...
def plot_voronoi_boundary(geneID, coord, count, 
    ...
    :param file: geneID; spatial coordinates (n, 2); 
                predicted cell class calls (n); prediction p-value.
    '''
    points = coord
    count = count
    labels = classLabel
...
def pdf_voronoi_boundary(geneID, coord, count, 
    ...
    :param file: spatial coordinates (n, 2); predicted cell class calls (n); 
    '''
    points = coord
    count = count
    labels = classLabel
...
def normalize_count_cellranger(data):
    ...
    :param file: data (m, n); A dataframe
    :rtype: data (m, n);
    '''
...
def cut_graph_general_zero(cellGraph, count, unary_scale_factor=100, 
                      smooth_factor=50, label_cost=10, algorithm='expansion'):
    '''
    Returns new labels and energy for the cut.
    ...
    :unary_scale_factor, scalar; smooth_factor, scalar; 
    :label_cost: scalar; 
    :rtype: label shape (n,); energy scalar.
    '''
    a = count.copy()
    gmm = find_mixture(a)
    unary_cost = compute_unary_cost_2(count, gmm, unary_scale_factor) # now set zero cluster cost
...
def compute_unary_cost_2(count, gmm, unary_scale_factor):
    unary_cost = compute_unary_cost_simple(count, gmm, unary_scale_factor)
    zero_index = np.where(unary_cost[:,0] == 0)[0]
    one_index = np.where(unary_cost[:,1] != 0)[0]
    if np.isnan(np.mean(unary_cost[np.where(unary_cost[:,1] < 0)[0],1])):
        cost = 0
    else:
        cost = int(np.mean(unary_cost[np.where(unary_cost[:,1] < 0)[0],1]))
    unary_cost[zero_index, 0] = cost
    unary_cost[zero_index, 1] = 0 - cost  
    return unary_cost

def compute_unary_cost_potts():
    '''
    Returns unary cost energy.
    
    :param points: count: shape (n,); means: shape(k,)

    :rtype: unary energy matrix.
    '''
    return np.square(count[:,np.newaxis]-gmm.means_.T)


def compute_unary_cost_L2(count, means):
    '''
    Returns unary cost energy.
...
def compute_p_CSR(newLabels, gmm, exp, cellGraph): #use 50% to speedup
    '''
    Returns p_value of the cut.
    
    :param points: newLabels: shape (n,); gmm: gmm object
                   ndarray shape (n ,3); alpha_list: shape (n,)
...
def compute_p_CSR_simple(newLabels, gmm, exp, cellGraph): #use 50% to speedup
    '''
    Returns p_value of the cut.
...
def compute_alpha_shape(points):
    '''
    Returns list of abnormal cells.
    
    :param points: ndarray shape (n ,32; 
...
def estimate_smooth_factor_genomewise_simple(locs, data_norm, unary_scale_factor=100, 
                      smooth_factor=50, label_cost=10, algorithm='expansion'):
    '''
    Returns calculated p-value and # of components from graph cut.
...
def cluster_spatial_genes(labels_matrix):
    return True
...
def visualize_spatial_genes(df, locs, data_norm, point_size= 0.5,
                            unary_scale_factor=100, algorithm='expansion'):
...
def dbScan(tsne_proj, z, threshold, eps=1):
    ...
    db = DBSCAN(eps=eps, min_samples=10).fit(tsne_proj_sel)
